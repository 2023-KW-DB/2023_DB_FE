...
const UsageNumberRanking = () => {
  ...
  useEffect(() => {
    ...
    (async() => {
      try{
        const response = await fetch(process.env.REACT_APP_API_URL + "/get-highest-usecount", {
          method: "GET",  
      ...
      <Table>
        <TableHead sx={{width: "100%"}}>
          <TableRow>
            ...
            <TableCell>
              <Typography variant="span">사용횟수</Typography>
            </TableCell>
          </TableRow>
        </TableHead>
            ...
            <>
              {showData.map((row, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    <Typography variant="span">{row.total_use_count}</Typography>
                  </TableCell>
                </TableRow>
              ))}
            </>
...
export default UsageNumberRanking;